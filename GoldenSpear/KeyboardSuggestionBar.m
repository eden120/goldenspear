...
@interface KeyboardSuggestionBar ()

@property (nonatomic, strong) KeyboardSuggestionBarView *suggestionBarView;
@property (nonatomic, assign) NSRange relevantContextRange;

@end

@implementation KeyboardSuggestionBar
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.suggestionBarView = [[KeyboardSuggestionBarView alloc] initWithFrame:[KeyboardSuggestionBar suggestionBarFrame]
                  ...
                  inModelNamed:(NSString *)modelName
{
    if ([textInputView respondsToSelector:@selector(setInputAccessoryView:)]) {
        ...
        textInputView.keyboardSuggestionBar = self;
...
- (void)dealloc
{
    ...
    self.suggestionBarView.suggestionBarController = nil;
    self.suggestionBarView = nil;
...
- (void)textChanged:(UIControl<UITextInput> *)textInputView
{
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(suggestionBar:relevantContextForInput:caretLocation:)]) {
        ...
        if([relevantContext isEqualToString:@""]){
            self.relevantContextRange = NSMakeRange(inputText.length, 0);
        }else{
