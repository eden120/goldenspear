...
@interface KeyboardCoreDataManager ()
...
@implementation KeyboardCoreDataManager
...
+ (instancetype)managerForModelName:(NSString *)dataModelName
{
    ...
    if (!sharedManager) {
        @synchronized (_sharedManagers) {
            [_sharedManagers setObject:[[KeyboardCoreDataManager alloc] initWithModelName:dataModelName] forKey:dataModelName];
...
{
    KeyboardCoreDataManager *sharedManager = [KeyboardCoreDataManager managerForModelName:dataModelName];
...
{
    if (_sharedManagers) {
        for (KeyboardCoreDataManager *manager in [_sharedManagers allValues]) {
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    if (!_persistentStoreCoordinator) {
        ...
        if (self.client) {
            KeyboardCoreDataIncrementalStore *incrementalStore =
            (KeyboardCoreDataIncrementalStore*)[_persistentStoreCoordinator addPersistentStoreWithType:[KeyboardCoreDataIncrementalStore type]
                                                                                     configuration:nil
                                                                                               URL:nil
                                                                                           options:nil
                                                                                             error:nil];
