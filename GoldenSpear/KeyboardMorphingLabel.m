...
#define kKeyboardKernFactorAttributeName @"kKeyboardKernFactorAttributeName"

@interface KeyboardMorphingLabel ()
...
@implementation KeyboardMorphingLabel
...
- (NSArray *)attributionStages
{
    if (!_attributionStages) {
        ...
        for (int i = 0; i < self.numberOfAttributionStages; i++) {
            ...
            attributionStage[kKeyboardKernFactorAttributeName] = [NSNumber numberWithFloat:1 - progress];
...
- (NSString *)prepareMorphing
{
    if (self.nextText) {
        ...
        if (self.text) {
            NSDictionary *mergeResult = [self.nextText keyboardMergeIntoString:self.text];
            
            newText = mergeResult[kKeyboardDictionaryKeyMergedString];
            self.additionRanges = mergeResult[kKeyboardDictionaryKeyAdditionRanges];
            self.deletionRanges = mergeResult[kKeyboardDictionaryKeyDeletionRanges];
        } else {
...
- (void)applyAttributionStage:(NSInteger)stage toString:(NSString *)aString
{
    ...
    void(^applyMutations)(NSArray *ranges, NSInteger offset) = ^(NSArray *ranges, NSInteger offset){
        for (NSValue *value in ranges) {
            ...
            CGFloat kernFactor = [attributionStage[kKeyboardKernFactorAttributeName] floatValue];
