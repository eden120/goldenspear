...
- (void)commonInit {
	...
	_titleColorsByState = [NSMutableDictionary dictionaryWithDictionary:@{@(UIControlStateNormal) : [UIColor grayColor]}];
...
- (void)reloadData {
	...
	if (totalButtons > 0 && self.selectedButtonIndex >= 0 && self.selectedButtonIndex < totalButtons && self.isShowSelected) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (isSelected && self.isShowSelected) {
		((id<HTHorizontalSelectionListCell>)cell).state = UIControlStateSelected;
	} else {
