...
-(void)initFrames {
    ...
    srhTitleLbl.text = _wardrobeName;
    SDWebImageManager *manager = [SDWebImageManager sharedManager];

        thumbAry = [[NSMutableArray alloc] init];
    for (int i = 0; i < [_contents count]; i ++) {
        GSBaseElement *element = (GSBaseElement *)[_contents objectAtIndex:i];
        NSString *imageURL = element.preview_image;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GSBaseElement *element = (GSBaseElement *)[_contents objectAtIndex:indexPath.row];
//    [cell.imageview sd_setImageWithURL:[NSURL URLWithString:element.preview_image]];
