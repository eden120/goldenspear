...
- (void)moveAngleToPosition:(CGFloat)x{
    self.angleLeadingConstraint.constant = x - self.angleImage.frame.size.width/2;
    self.bottomAngleLeadingConstraint.constant = x - self.angleImage.frame.size.width/2;
...
- (void)pushOptionAtIndex:(UIButton*)sender{
    [self.viewDelegate optionsView:self selectOptionAtIndex:sender.tag];
}
...
- (void)setNewHeight:(CGFloat)newHeight{
    [_viewDelegate optionsView:self heightChanged:newHeight+46];
}

- (void)showUpToDown:(BOOL)upToDownOrNot{
    isUpToDown = upToDownOrNot;
    if(isUpToDown){
        _angleImage.hidden = NO;
        _bottomAngleImage.hidden = YES;
        self.containerBgTopConstraint.constant = 30;
        self.containerBgBottomConstraint.constant = 0;
    }else{
        _angleImage.hidden = YES;
        _bottomAngleImage.hidden = NO;
        self.containerBgTopConstraint.constant = 0;
        self.containerBgBottomConstraint.constant = 30;
    }
    [self layoutIfNeeded];
}
