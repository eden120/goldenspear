...
@interface FilterSearchBrandViewController ()
...
@implementation FilterSearchBrandViewController
{
...
- (void)viewDidLoad {
    ...
    selectedProductTermsObjectList = [[NSMutableArray alloc] init];
    ...
    iGenderSelected = 0;
    brands = [[CoreDataQuery sharedCoreDataQuery] getAllBrands];
    brandsSelectedProductCategory = brands;
...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    selectedProductTermsObjectList = [[NSMutableArray alloc]initWithArray:_selectedBrands];
...
-(void) createGenderSlideView
{
    if (genderSlideView == nil)
    {
        ...
        [genderSlideView initSlideButtonWithButtons:selectedProductTermsObjectList andDelegate:self];

        [genderSlideView setSpaceBetweenButtons:0];
        [genderSlideView setBShowShadowsSides:YES];
        [genderSlideView setBShowPointRight:YES];
        [genderSlideView setBButtonsCentered:NO];
        [genderSlideView setLastState];
...
//                NSSortDescriptor *sortDescriptorPriority = [[NSSortDescriptor alloc] initWithKey:@"priority" ascending:NO];
    ...
    brandsSelectedProductCategory = [NSMutableArray arrayWithArray:[brands sortedArrayUsingDescriptors:[NSArray arrayWithObjects:sortDescriptorName, nil]]];
...
-(void) showBrandsProductCategoryCollectionView:(long) iLevel
{
    ...
    if (brandsSelectedProductCategory != nil)
    {
        ...
        // create the alphabet
        [self initAlphabetBrands:brands];
    }
    brandsSelectedProductCategory = brands;
    self.lblNoFilters.hidden = !self.mainCollectionView.hidden;
...
{
    [selectedProductTermsObjectList removeObjectAtIndex:buttonEntry];
    [self createGenderSlideView];
    [self.secondCollectionView reloadData];
}
...
- (NSInteger)numberOfItemsInSection:(NSInteger)section forCollectionViewWithCellType:(NSString *)cellType
{
    ...
    {
        return brandsSelectedProductCategory.count;
    }
    return 0;
}
...
- (NSArray *)getContentForCellWithType:(NSString *)cellType AtIndexPath:(NSIndexPath *)indexPath
{
    if ([cellType isEqualToString:@"FeatureCell"] || [cellType isEqualToString:@"ProductCategoryCell"])
    {
        ...
        if ([selectedProductTermsObjectList containsObject:brand]) {
...
- (void)actionForSelectionOfCellWithType:(NSString *)cellType AtIndexPath:(NSIndexPath *)indexPath
{
    if ([cellType isEqualToString:@"FeatureCell"] || [cellType isEqualToString:@"ProductCategoryCell"])
    {
        ...
        NSString* sName = brand.name;
        
        NSString * sNameTrimmed = [sName stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        
        if (brand != nil) {
            if ([selectedProductTermsObjectList containsObject:brand]) {
                [selectedProductTermsObjectList removeObject:brand];
            } else {
...
//            if ([selectedProductTermsList containsObject:sNameTrimmed]) {
//                
//                [selectedProductTermsList removeObject:sNameTrimmed];
//                NSString *str =  sNameTrimmed;
//                if ([arr count] > 0) {
...
//                [self createGenderSlideView];
//            } else {
//                [selectedProductTermsList addObject:sNameTrimmed];
//                NSString *str =  sNameTrimmed;
//                if ([arr count] > 0)
