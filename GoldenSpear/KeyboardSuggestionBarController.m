//
//  KeyboardSuggestionBarController.m
//  GoldenSpear
//
...
@implementation KeyboardSuggestionBarController
...
{
    return((KeyboardSuggestionBarView *)self.collectionView).numberOfSuggestionFields;
}

- (NSString *)attributeName
{
    return((KeyboardSuggestionBarView *)self.collectionView).attributeName;
}
...
{
    NSInteger numberOfSuggestionFields = [self numberOfSuggestionFields];
    CGFloat width = (self.collectionView.frame.size.width - (numberOfSuggestionFields - 1) * kKeyboardSuggestionBarCellPadding) / (CGFloat)numberOfSuggestionFields;
    ...
    if (numberOfSuggestionFields % 2 == 0) {
        ...
        if (indexPath.row == floorf(numberOfSuggestionFields / 2)) {
            ...
            width = (self.collectionView.frame.size.width - (numberOfSuggestionFields - 1) * kKeyboardSuggestionBarCellPadding - maxWidth) / (CGFloat)(numberOfSuggestionFields - 1);
            return CGSizeMake(floorf(width), height);
        }
    }
}
...
#pragma mark - KeyboardCoreDataManagerDataSource

- (NSString *)modelName
{
    return ((KeyboardSuggestionBarView *)self.collectionView).modelName;
}

- (NSString *)entityName
{
    return ((KeyboardSuggestionBarView *)self.collectionView).entityName;
}
...
{
    KeyboardSuggestionBarCell *suggestionBarCell = (KeyboardSuggestionBarCell *)cell;
    ...
    @try {
        ...
        if (object) {
            ...
            @throw [NSException exceptionWithName:@"KeyboardObjectNotFoundException"
