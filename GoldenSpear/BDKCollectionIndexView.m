...
#define kWidthLetterZoom 96
#define kHeightLetterZoom 64
...
//    NSLog(@"Creating Letter");
    lblLetterZoom = [[UILabel alloc] initWithFrame:CGRectMake(0,0, kWidthLetterZoom, kHeightLetterZoom)];
    [self addSubview:lblLetterZoom];
    [self bringSubviewToFront:lblLetterZoom];
    
    UIImage *img = [UIImage imageNamed:@"bubble"];
    CGSize imgSize = lblLetterZoom.frame.size;
    ...
    lblLetterZoom.backgroundColor = [UIColor colorWithPatternImage:newImage];
    lblLetterZoom.textAlignment = NSTextAlignmentCenter;
    lblLetterZoom.hidden = YES;
...
- (UIFont *)fontZoom {
    return _fontZoom ? _fontZoom : [UIFont boldSystemFontOfSize:12.0f];
}
...
- (void)setFontZoom:(UIFont *)font {
    _fontZoom = font;
    lblLetterZoom.font = _fontZoom;
}

-(void) setFontZoomColor:(UIColor *)fontZoomColor{
    _fontZoomColor = fontZoomColor;
    lblLetterZoom.textColor = _fontZoomColor;
}
...
- (void)setBackgroundVisibility:(BOOL)flag {
    ...
    lblLetterZoom.hidden = (alpha <= 0.01);
...
- (void)handleGesture:(UIGestureRecognizer *)recognizer {
    ...
    CGRect frame = CGRectMake(-kWidthLetterZoom, point.y - (kHeightLetterZoom / 2.0), kWidthLetterZoom, kHeightLetterZoom);
    [lblLetterZoom setFrame:frame];
    lblLetterZoom.text = self.currentIndexTitle;
